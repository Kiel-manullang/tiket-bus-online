...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold">Edit Armada</h1>
            <p class="text-slate-400">Perbarui informasi armada: {{ $bus->name }}</p>
        </div>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.buses.update', $bus) }}" method="POST" enctype="multipart/form-data" class="max-w-3xl">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           id="facilities" 
                           name="facilities" 
                           value="{{ old('facilities', $bus->facilities) }}" 
                ...
                <div>
                    ...
                    <!-- Current Image -->
                    @if($bus->image)
                    <div class="mb-4 p-4 rounded-xl bg-navy-900/50 border border-white/10">
                        <p class="text-xs text-slate-400 mb-2">Gambar saat ini:</p>
                        <div class="flex items-center gap-4">
                            <img src="{{ asset('storage/' . $bus->image) }}" alt="{{ $bus->name }}" class="w-20 h-20 rounded-lg object-cover">
                            <div>
                                <p class="text-sm text-slate-300">{{ basename($bus->image) }}</p>
                    ...
                    <div id="imageDropZone" class="relative border-2 border-dashed border-white/20 hover:border-sky-500/50 rounded-xl p-6 text-center transition-all cursor-pointer">
                        ...
                        <div id="imagePlaceholder">
                            ...
                            <p class="text-sm font-medium mb-1">Klik untuk upload gambar baru</p>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <label class="relative cursor-pointer">
                            <input type="radio" name="status" value="active" class="peer sr-only" {{ old('status', $bus->status) === 'active' ? 'checked' : '' }} required>
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="status" value="maintenance" class="peer sr-only" {{ old('status', $bus->status) === 'maintenance' ? 'checked' : '' }}>
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="status" value="inactive" class="peer sr-only" {{ old('status', $bus->status) === 'inactive' ? 'checked' : '' }}>
            ...
            <div class="bg-white/5 border-t border-white/10 px-6 py-4 flex items-center justify-between">
                <form action="{{ route('admin.buses.destroy', $bus) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus armada ini?')">
                    @csrf
                    @method('DELETE')
